...
| `<leader>mp` | n | 切换 Markdown 预览（私密浏览器） |
...
| `<leader>tz` | v | 翻译选中内容 → 中文 |
| `<leader>te` | v | 翻译选中内容 → 英文 |
...
| `k` / `<Up>` | n, x | 向上移动（兼容折叠行） |
| `<C-h/j/k/l>` | n | 跳到左/下/上/右窗口 |
| `<C-Up/Down>` | n | 增减窗口高度 |
| `<C-Left/Right>` | n | 增减窗口宽度 |
| `<A-j>` / `<A-k>` | n, i, v | 上下移动行 |
...
| 快捷键 | 模式 | 说明 |
|--------|------|------|
| `<leader>uf` / `uF` | n | 切换自动格式化（全局/Buffer） |
| `<leader>us` | n | 切换拼写检查 |
| `<leader>uw` | n | 切换自动换行 |
| `<leader>ul` / `uL` | n | 切换行号/相对行号 |
| `<leader>ud` | n | 切换诊断 |
...
| `<leader>sr` | n | 搜索寄存器 |
| `<leader>sw` | n | 搜索光标下单词（根目录） |
| `<leader>sW` | n | 搜索光标下单词（cwd） |
| `<leader>s/` | n | 搜索历史 |
| `<leader>/` | n | 模糊搜索当前 Buffer |
| `<leader>:` | n | 命令历史 |
| `<leader>,` | n | 切换 Buffer |
| `<leader><space>` | n | 查找文件 |
